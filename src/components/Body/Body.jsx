      ...
      <div className="countryListContainer">
        {countryList?.length > 0 &&
          countryList?.map((country) => (
            <div key={country.name.official} className="country" onClick={() => navigate(`/${country.cca3}`)}>
        ...
        {countryList?.length === 0 && (
          <p className="fw-800">
            Could not find a country with the filter selected. Please try using
            "All"
          </p>
        )}
      </div>
